"""
Prepare APHREH-ADSMap input files for the first feasibility pilot.
...
PILOT_POLLUTANT = "PM25"
PILOT_OUTCOME = "Respiratory"
...
def assert_no_missing_columns(df: pd.DataFrame, required_columns: list[str], table_name: str) -> None:
...
def load_study_municipalities() -> pd.DataFrame:
    ...
    if study["Municipality_code"].nunique() != 37:
        raise ValueError(
            f"Expected 37 study municipalities, found {study['Municipality_code'].nunique()}."
        )

    if study["Municipality_key"].duplicated().any():
        duplicated = study.loc[study["Municipality_key"].duplicated(), "Municipality"].tolist()
...
def build_exposure_data(study: pd.DataFrame, selected_dates: pd.DatetimeIndex) -> pd.DataFrame:
    ...
    unmatched = exposure_long.loc[exposure_long["Municipality_code"].isna(), ["Area", "Municipality", "Municipality_key"]]
    ...
    # Average possible duplicate Date-Municipality rows, even if duplicates should already be absent.
    exposure_daily = (
        exposure_long
    ...
    if missing_values > 0:
        ...
        raise ValueError(
            f"Exposure data contain {missing_values} missing values. Missing by municipality:\n"
...
def build_outcome_data(study: pd.DataFrame, selected_dates: pd.DatetimeIndex) -> pd.DataFrame:
...
def build_bsa_data(study: pd.DataFrame) -> pd.DataFrame:
    ...
    # Identity geometry for the pilot: each municipality has artificial area = 1.
    # This works together with SRCBSA.csv where the intersection area is also 1.
...
def build_srcbsa_data(study: pd.DataFrame) -> pd.DataFrame:
    ...
    APHREH cross_grid_computation() expects both:
        - cross_area_field, here "Area"
        - area_field, here "BSA_AREA"

    inside the cross-grid table.
    """
    srcbsa = pd.DataFrame(
        {
...
def main() -> None:
    ...
    print("=" * 100)
    print("Preparing APHREH-ADSMap pilot inputs")

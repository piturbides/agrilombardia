...
def run_pm25_definitivo_non_covid_analysis():
    ...
    monthly_climatology_pivot = monthly_climatology_pivot[STATION_ORDER]
    monthly_climatology_pivot.plot(
        marker="o",
        figsize=(9, 5),
    ...
    seasonal_climatology_pivot = seasonal_climatology_pivot[STATION_ORDER]
    seasonal_climatology_pivot.plot(
        kind="bar",
        figsize=(8, 5),
        color=[STATION_COLORS[station] for station in STATION_ORDER]
    )
    ...
    # Seasonal boxplot
    plt.figure(figsize=(8, 5))
    seasonal_boxplot = seasonal.copy()
    seasonal_boxplot["Station"] = pd.Categorical(
        seasonal_boxplot["Station"],
        categories=STATION_ORDER,
        ordered=True
    )
